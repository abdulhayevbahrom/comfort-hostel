...
import { printPayments } from "./printPayments";
...
export function PaymentsPage({ currentEmployee }) {
  ...
  const [getAllPayments, { isFetching: isPreparingPrint }] =
    useLazyGetAllPaymentsQuery();
  ...
  const [createDepositPayment, { isLoading: savingDeposit }] =
    useCreateDepositPaymentMutation();
  const [deleteDepositPayment, { isLoading: deletingDeposit }] =
    useDeleteDepositPaymentMutation();
  ...
  const selectedDepositStudent = depositStudents.find(
    (item) => item._id === selectedDepositStudentId,
  );
  const partsTotal = Object.values(paymentParts).reduce(
    (sum, value) => sum + (Number(value) || 0),
    0,
  );
  const buildParts = (values) =>
    Object.entries(values.paymentParts || {})
      .filter(([, amount]) => Number(amount) > 0)
      .map(([partMethod, partAmount]) => ({
        method: partMethod,
        amount: Number(partAmount),
        paidAt: values.paymentDates?.[partMethod]?.toISOString(),
      }));
  ...
  const submit = async (values) => {
    try {
      if (editingPayment) {
        ...
        const parts = await uploadReceiptParts(
          buildParts(values),
          values.receiptFiles,
        );
  ...
  const remove = async (payment) => {
    try {
      const ids = payment.sourcePaymentIds?.length
  ...
  const employeeName = (employee) =>
    employee
      ...
      : "Noma’lum xodim";
  const lastEditor = (payment) =>
    [...(payment.auditHistory || [])]
      .reverse()
      .find((entry) => entry.action === "updated")?.performedBy;
  const actionNames = {
    created: "To‘lov qabul qilindi",
    updated: "To‘lov tahrirlandi",
  ...
  const printAllPayments = async () => {
    try {
      const printData = await getAllPayments(filters).unwrap();
      printPayments(
        groupPayments(printData?.payments || []),
        filters,
        settingsData?.settings,
      );
    } catch (requestError) {
  ...
  return (
    <div className="payments-page">
        ...
        <>
          <section className="payment-stats">
            {[
              ...
              [
                "debt",
                summary.isFuturePeriod
                  ? "Qarzdorlik boshlanmagan"
                  : "Qarzdorlik",
          ...
          <section className="payment-card">
            ...
            <div className="payment-filters">
              <div className="payment-search">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    updateFilters((old) => ({ ...old, search: value }));
                  }}
                />
                <button
              ...
              <div className="payment-filter-options">
                <Select
                  ...
                  onChange={(value) =>
                    updateFilters((old) => ({ ...old, method: value || "" }))
                  }
            ...
            {error && (
              <div className="form-error">{apiErrorMessage(error)}</div>
            )}
            {isLoading ? (
              ...
              <div
                className={`payment-table-wrap ${isFetching ? "refreshing" : ""}`}
              >
                      ...
                      >
                        ...
                        <td data-label="Oy">
                          <span className="payment-period">
                            {payment.isDeposit
                              ? "Depozit"
                              : payment.allocations?.[0]?.installment
                                  ?.periodKey || "—"}
                          </span>
                        </td>
                        <td data-label="Sana">
                        ...
                        <td data-label="To‘lov usuli">
                          ...
                          {!payment.isGrouped &&
                            ...
                            payment.fundHolder && (
                              <small>
                                {payment.fundHolder === "organization"
                                  ? "Tashkilot hisobi"
                                  : "Kassirning shaxsiy hisobi"}
                              </small>
                            )}
                        </td>
                        ...
                        <td data-label="Tahrirlagan">
                          {lastEditor(payment) ? (
                            <strong>{employeeName(lastEditor(payment))}</strong>
                          ) : (
                            "—"
                          )}
                        </td>
                        <td data-label="Summa">
                          ...
                          {payment.status === "cancelled" && (
                            <small className="payment-cancelled-badge">
                              Bekor qilingan
                            </small>
                          )}
                        </td>
                          ...
                          <div className="payment-row-actions">
                            ...
                            {!payment.isGrouped && (
                              <button
                                ...
                                aria-label="To‘lov amallari tarixini ko‘rish"
                                onClick={() => setHistoryPayment(payment)}
                              >
                                <svg viewBox="0 0 24 24" aria-hidden="true">
                                  <path d="M12 8v5l3 2M4.5 9A8 8 0 1 1 4 12M4 5v4h4" />
                                </svg>
                              </button>
                            )}
                            {isOwner && payment.status !== "cancelled" && (
                              <>
                                {!payment.isDeposit && !payment.isGrouped && (
                                  <button
            ...
            {(data?.pagination?.total || 0) > 25 && (
              <div className="payment-pagination">
                <span>Jami {data.pagination.total} ta tranzaksiya</span>
                <Pagination
      ...
      <Modal
        ...
        footer={null}
        title="Filterlar"
        rootClassName="payment-filters-modal"
        destroyOnHidden
      >
        ...
        <div className="payment-audit-list">
          {(historyPayment?.auditHistory?.length
            ...
            : [
                {
                  action: "created",
                  performedBy: historyPayment?.receivedBy,
                  performedAt: historyPayment?.createdAt,
                  after: historyPayment,
                },
              ]
          ).map((entry, index) => (
            <article key={entry._id || `${entry.action}-${index}`}>
              ...
              <div>
                ...
                {entry.action === "updated" && (
                  <small>
                    {money(entry.before?.amount)} → {money(entry.after?.amount)}{" "}
                    · {methods[entry.before?.method]} →{" "}
                    {methods[entry.after?.method]}
                  </small>
                )}
                {entry.action === "cancelled" && (
                  <small>
                    Bekor qilingan summa: {money(entry.before?.amount)}
                  </small>
                )}
                <time>
                  {dayjs(entry.performedAt || historyPayment?.createdAt).format(
                    "DD.MM.YYYY HH:mm",
                  )}
                </time>
              </div>
          ...
          {!editingPayment && (
            <Form.Item name="paymentKind" label="To‘lov yo‘nalishi">
              <Segmented
                ...
                block
                options={[
                  { value: "contract", label: "Shartnoma to‘lovi" },
                  { value: "deposit", label: "Depozit to‘lovi" },
                ]}
              />
            </Form.Item>
          )}
            ...
            <>
              ...
              >
                <Select
                  ...
                  options={depositStudents.map((item) => ({
                    value: item._id,
                    label: `${item.fullName} — ${money(item.balance)} depozit qarzi`,
                  }))}
                />
              </Form.Item>
              {selectedDepositStudent && (
                <div className="selected-contract">
                  <div>
                    <small>Talaba</small>
                    <b>{selectedDepositStudent.fullName}</b>
                  </div>
                  <div>
                    <small>Depozit summasi</small>
                    <b>
                      {money(
                        Math.max(
                          Number(selectedDepositStudent.depositAmount || 0),
                          700000,
                        ),
                      )}
                    </b>
                  ...
                  <div>
                    <small>Qoldiq</small>
                    <b>{money(selectedDepositStudent.balance)}</b>
                  </div>
                </div>
              )}
            </>
          ...
          {editingPayment ? (
            ...
            <div className="payment-split-fields">
              ...
              <div className="payment-method-rows">
                {Object.entries(methods).map(([partMethod, label]) => (
                  <div
                    className={`payment-method-row method-${partMethod}`}
                    key={partMethod}
                  >
                    <Form.Item
                      name={["paymentParts", partMethod]}
                      label={label}
                    >
                      <InputNumber
                        min={0}
                    ...
                    <Form.Item
                      name={["paymentDates", partMethod]}
                      label="To‘lov sanasi va vaqti"
                      rules={
                        Number(paymentParts[partMethod] || 0) > 0
                          ? [
                              {
                                required: true,
                    ...
                    >
                      <DatePicker
                        disabled={Number(paymentParts[partMethod] || 0) <= 0}
                        showTime
              ...
              <div className="selected-contract">
                ...
                <div>
                  ...
                  <b>
                    {money(
                      paymentKind === "deposit"
                        ? selectedDepositStudent?.balance
                        : availableBalance,
                    )}
                  </b>
                </div>
