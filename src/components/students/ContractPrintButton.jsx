...
export function ContractPrintButton({ contract, student, organization }) {
  ...
  const print = async () => {
    ...
    try {
      ...
      printDocument.open()
      printDocument.write('<!doctype html><html><head><meta charset="utf-8"><title>Shartnoma</title><style>@page{size:A4 portrait;margin:0}html,body{margin:0;padding:0}img{display:block;width:209mm;height:auto;margin:0 auto;break-inside:avoid;page-break-inside:avoid}img+img{break-before:page;page-break-before:always}@media screen{body{background:#303438;padding:12px}img{max-width:100%;margin:0 auto 12px;background:#fff}}</style></head><body></body></html>')
      printDocument.close()
  ...
  return <><button onClick={print} disabled={printing} aria-label="Chop etish" title="Chop etish"><PrinterOutlined /></button><div className="contract-download-source" aria-hidden="true"><ContractDocument ref={documentRef} contract={contract} student={student} organization={organization} /></div></>
}
