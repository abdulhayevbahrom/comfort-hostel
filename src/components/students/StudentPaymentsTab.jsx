...
import { apiErrorMessage, useCreateDepositPaymentMutation, useCreatePaymentMutation, useDeleteDepositPaymentMutation, useDeletePaymentMutation, useGetGeneralSettingsQuery, useGetStudentPaymentsQuery, useUpdatePaymentMutation } from '../../store/baseApi'
...
import { ReceiptImageLink } from '../payments/ReceiptImageLink'
...
export function StudentPaymentsTab({ student, currentEmployee }) {
  ...
  const depositPaid = (data?.payments || []).filter((payment) => payment.isDeposit && payment.status !== 'cancelled' && !payment.cancelledAt).reduce((sum, payment) => sum + Number(payment.amount || 0), 0)
  const depositRequired = ['none', 'money'].includes(student.depositType) && !student.depositReturnedAt ? Math.max(Number(student.depositAmount || 0), 700000) : 0
  ...
  const remove = async (payment) => {
    try { const ids = payment.sourcePaymentIds?.length ? payment.sourcePaymentIds : [payment.id]; for (const id of ids) { if (payment.isDeposit) await deleteDepositPayment({ studentId: payment.student?.id || student.id, paymentId: id }).unwrap(); else await deletePayment(id).unwrap() } toast.success('To‘lov bekor qilindi') }
    catch (requestError) { toast.error(apiErrorMessage(requestError)) }
  }
  const openPayment = () => { const contract = contractOptions[0]; const installment = (data?.installments || []).find((item) => String(item.contract) === contract?._id && item.paidAmount < item.amount); paymentForm.setFieldsValue({ paymentKind: contract ? 'contract' : 'deposit', contract: contract?._id, installment: installment?._id, paymentParts: { cash: 0, online: 0, card: 0, bank: 0 }, paymentDates: {}, receiptFiles: {}, payerType: '', note: '' }); setPaymentOpen(true) }
